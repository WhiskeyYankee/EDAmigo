...
#' @param special_user_level An indicator of whether the user will provide input when removing special characters or if the user would like to fully automate the process, 1 indicates user interaction.
#' @param missing_user_level An indicator of whether the user will provide input when handling missing values or if the user would like to fully automate the process, 1 indicates user interaction.
#'
...
autoClean <- function(df, factor_tol = NULL, user_tol = 30, vals = "[^0-9A-Za-z.,[:space:]-]", no_drop = FALSE,  no_impute = FALSE, drop_col_tol = 50, drop_row_tol = NULL, user_level = 0){
  ...
  clean <- handleMissing(typed_df, no_drop, drop_col_tol, drop_row_tol, user_level)
